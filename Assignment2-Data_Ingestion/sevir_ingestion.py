...
DATA_CATALOG = ('s3://seviringestion/CATALOG.csv')

DATA_sevir = ('s3://seviringestion/data')

DATA_interim = ('s3://seviringestion/data')
...
class SEVIRSequence(Sequence):
    ...
    def _open_files(self,verbose=True):
        ...
        for f in hdf_filenames:
            ...
            self._hdf_files[f] = h5py.File(s3.open(self.sevir_data_home+'/'+f,'rb'))
...
def read_write_chunks( filename, generator, n_chunks ):
    ...
    s3 = s3fs.S3FileSystem()
    #s3 = boto3.resource('s3')
    with s3.open(filename,'a') as s3file:
        with h5py.File(s3file, 'w') as hf:
...
def handler(event, context):
    ...
    x_test,y_test = read_data('s3://seviringestion/data/nowcast_testing.h5', end=10)
    print(x_test.shape)
    print(y_test.shape)
    return {
