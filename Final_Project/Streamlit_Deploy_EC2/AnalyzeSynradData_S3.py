...
def Synrad_visualize_result(Y,idx,ax):
...
def Nowcast_visualize_result(x_test,y_test,idx,ax):
    fs=10
...
if application == "Nowcast":
    ...
    if st.sidebar.checkbox('Show Nowcast input data size'):
        st.sidebar.subheader('Nowcast Input data size')
        st.sidebar.write(x_test.shape)
    if st.sidebar.checkbox('Show Nowcast input data'):
        st.sidebar.subheader('Nowcast Input data')
        st.sidebar.write(x_test)            
    if st.sidebar.checkbox('Show Nowcast output data size'):
        st.sidebar.subheader('Nowcast output data size')
        st.sidebar.write(y_test.shape)
    if st.sidebar.checkbox('Show Nowcast output data'):
        st.sidebar.subheader('Nowcast output data')
        st.sidebar.write(y_test)          
    ...
    if st.button("Nowcast"):
        ...
        fig,ax = plt.subplots(4,3,figsize=(20,8), gridspec_kw={'width_ratios': [1,.2,1]})
        Nowcast_visualize_result(x_test,y_test,idx,ax)
